...
class NetflixNarcApp(App[None]):
    ...
    async def _startup_sync_sequence(self) -> None:
        ...
        await self.rebuild_table(evaluate=False)
        self._set_loading(state=False)
        backend = create_storage_backend(self.settings)
        if backend is not None:
            self.sync_engine = SyncEngine(backend, self.evidence_locker, self.settings)
    ...
    def handle_onboarding_complete(self, result: OnboardingResult | None) -> None:
        """Handle onboarding completion."""
        if result is None:
            self.exit()  # User cancelled first-run onboarding
            return
        ...
        persist_key = api_key if api_key is not None else SecretStr("")
        ...
        update_env_file(
            ...
            extra_env={"SCORING_MODE": str(result.scoring_mode.value)},
        )

        # Initialize provider if we now have a key
        if api_key and provider and persist_key.get_secret_value():
            try:
                self.rating_provider = get_rating_provider(
                    settings=self.settings, cache_dir=self.cache_dir
                )
