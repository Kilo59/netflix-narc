...
7. **CI handles release builds**: Do NOT attempt to run package builds (`uv build`), binary builds (`invoke build-binary`), or `--offline` builds when creating a release. The GitHub Actions release workflow (`.github/workflows/release.yml`) builds all wheels, sdist packages, and cross-platform binaries automatically when a `v*` tag is pushed to `origin`. Release preparation only requires quality validation (`ruff`, `mypy`, `pytest`), bumping `pyproject.toml`, updating `CHANGELOG.md`, tagging, and pushing `main --tags`.
