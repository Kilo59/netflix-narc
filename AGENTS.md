...
7. **Release Workflow (Two-Phase & CI-Driven)**:
   ...
   - **Phase 1 (Pre-Release PR)**: Create a `release/vX.Y.Z` branch, run quality checks (`ruff`, `mypy`, `pytest`), bump `pyproject.toml` version, update `CHANGELOG.md` (move unreleased items to dated header), commit (`chore: prepare vX.Y.Z release`), open a PR (`gh pr create`), and merge to `main`.
   - **Phase 2 (Tag, Publish & Post-Release Narrative)**: Pull merged `main`, create an annotated tag (`git tag -a vX.Y.Z`), push tags (`git push origin main --tags`) to trigger CI publishing, and finally update the GitHub Release notes (`gh release edit vX.Y.Z`) with a high-level narrative and context beyond the raw changelog bullets.
