...
7. **Release Workflow (Two-Phase & CI-Driven)**:
   - Do NOT attempt local package (`uv build`) or binary (`invoke build-binary`) builds during release preparation — CI (`.github/workflows/release.yml`) builds all wheels, sdist packages, and PyApp binaries automatically when a `v*` tag is pushed.
   - **Phase 1 (Pre-Release PR & Merge)**: Because release commits do not already exist on `main`, create a dedicated release branch (`git checkout -b release/vX.Y.Z`), run quality checks (`ruff`, `mypy`, `pytest`), bump `pyproject.toml` version, update `CHANGELOG.md` (move unreleased items to a dated header), commit, push branch, open a PR (`gh pr create`), and merge to `main` so that all release artifacts exist on `main` before tagging.
   - **Phase 2 (Tag, Publish & Post-Release Narrative)**: Pull merged `main`, create an annotated tag (`git tag -a vX.Y.Z`), push `main` and tags (`git push origin main --tags`) to trigger CI publishing, and finally update the GitHub Release notes (`gh release edit vX.Y.Z` or GitHub web UI) with a high-level narrative and context beyond raw changelog bullets.
