...
7. **Release Workflow (Two-Phase & CI-Driven)**:
   ...
   - **Phase 1 (Pre-Release Preparation)**: Run quality checks (`ruff`, `mypy`, `pytest`), bump `pyproject.toml` version, update `CHANGELOG.md` (move unreleased items to a dated header), commit, and ensure all release artifacts exist on `main` (`git commit -am "chore: release vX.Y.Z"`). A dedicated branch is not required as long as artifacts are present on `main`.
   - **Phase 2 (Tag, Publish & Post-Release Narrative)**: Create an annotated tag on `main` (`git tag -a vX.Y.Z`), push `main` and tags (`git push origin main --tags`) to trigger CI publishing, and finally update the GitHub Release notes (`gh release edit vX.Y.Z` or GitHub web UI) with a high-level narrative and context beyond raw changelog bullets.
