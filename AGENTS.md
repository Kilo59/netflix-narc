...
7. **Release Workflow (Two-Phase & CI-Driven)**:
   ...
   - **Phase 1 (Pre-Release Preparation)**: Run quality checks (`ruff`, `mypy`, `pytest`), bump `pyproject.toml` version, update `CHANGELOG.md` (move unreleased items to a dated header), commit, and ensure all release artifacts exist on `main` (`git commit -am "chore: release vX.Y.Z"`).
